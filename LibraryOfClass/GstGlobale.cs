...
namespace LibraryOfClass
{
    ...
    public class GstGlobale
    {
        ...
        public Dictionary<string, int> getNbMedPertnonPert(int depot_legal)
        {
            gst_bdd = new GstBDD();
            Dictionary<string, int> lesDatas = new Dictionary<string, int>();

            int nbTotal = gst_bdd.getTotalNbMedoc();
            int nbPert = gst_bdd.getNbMedPert(depot_legal);
            int nbNonPert = nbTotal - nbPert;


            lesDatas.Add("Perturbateurs", nbPert);
            lesDatas.Add("NonPerturbateur", nbNonPert);
