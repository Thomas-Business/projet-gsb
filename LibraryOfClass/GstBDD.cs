...
namespace LibraryOfClass
{
    public class GstBDD
    {
        ...
        public void ModifierMedicament(Medicament leMedicament)
        {
            ...
            string contreIndic = leMedicament.Med_contreindic.Replace("'", " ").Replace('"', ' ');
            //double prix = leMedicament.Med_prixechantillon;
            double prix = Convert.ToDouble(leMedicament.Med_prixechantillon.ToString());
            //var test = leMedicament.Med_prixechantillon.ToString();
            //var test2 = Convert.ToDouble(test);

            cmd = new MySqlCommand("UPDATE `medicament` SET `med_nomcommerciale`= '" + nomCommercial + "',`fam_code`= " + leMedicament.LaFamille.Fam_code + ",`med_composition`= '" + composition + "',`med_effets`= '" + effet + "',`med_contreindic`= '" + contreIndic + "',`med_prixechantillon`= '" + prix.ToString().Replace(',', '.') + "' WHERE medicament.med_depotlegal = " + leMedicament.Med_depotlegal, cnx);
            dr = cmd.ExecuteReader();
            dr.Close();
        }
        ...
        public Dictionary<string, int> getNbMedocByFam()
        {
            Dictionary<string, int> lesDatas = new Dictionary<string, int>();

            cmd = new MySqlCommand("SELECT fam_libelle,COUNT(med_depotlegal) FROM medicament INNER JOIN famille ON medicament.fam_code = famille.fam_code GROUP BY medicament.fam_code;", cnx);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        public int getNbMedPert(int depot_legal)
        {
            int nb = 0;

            cmd = new MySqlCommand("SELECT COUNT(med_perturbateur) FROM interagir INNER JOIN medicament ON med_med_perturbe = med_depotlegal WHERE med_depotlegal = " + depot_legal + " GROUP BY med_med_perturbe;", cnx);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
               nb = Convert.ToInt32(dr[0]);
        ...
        public int getTotalNbMedoc()
        {
            int nb = 0;
            cmd = new MySqlCommand("SELECT COUNT(med_depotlegal) FROM medicament;", cnx);
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                nb = Convert.ToInt32(dr[0]);
        ...
        //récup les medicament dons les prix sont les plus élevé(10max)
        public Dictionary<string, double> getMedPrixEleve()
        {
            Dictionary<string, double> lesDatas = new Dictionary<string, double>();

            cmd = new MySqlCommand("SELECT `med_nomcommerciale`, `med_prixechantillon` FROM `medicament` ORDER BY med_prixechantillon DESC LIMIT 10", cnx);
            dr = cmd.ExecuteReader();
            dr.Read();
            while (dr.Read())
            {
                lesDatas.Add(dr[0].ToString(), Math.Round(Convert.ToDouble(dr[1]), 2));
